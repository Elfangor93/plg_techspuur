...
$sourceDir = realpath($rootDir . '/plugin');
...
if(isset($argv[2]) && strtolower($argv[2]) === 'fix')
{
  $doFix = true;
}

// If script is called with "details" argument → enable printing
if(isset($argv[3]) && strtolower($argv[3]) === 'details')
{
  $details = true;
}
...
if($content === false)
{
  throw new \Exception('Cannot read file: ' . basename($file) . PHP_EOL . PHP_EOL, 1);
}
